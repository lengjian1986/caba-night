...
class AdminTokenService
{
    ...
    public static function setToken($adminId, $terminal, $multipointLogin = 1)
    {
        ...
        // expire_time = 0 means the admin token does not expire automatically.
        $expireDuration = (int)Config::get('project.admin_token.expire_duration');
        $expireTime = $expireDuration === 0
            ? 0
            : $time + $expireDuration;
        ...
        //token处理
        if ($adminSession) {
            if (((int)$adminSession->expire_time !== 0 && $adminSession->expire_time < $time) || $multipointLogin === 0) {
                //清空缓存
    ...
    public static function overtimeToken($token)
    {
        ...
        //延长token过期时间。expire_time = 0 means permanent login.
        $expireDuration = (int)Config::get('project.admin_token.expire_duration');
        $adminSession->expire_time = $expireDuration === 0
            ? 0
            : $time + $expireDuration;
        $adminSession->update_time = $time;
        $adminSession->save();
